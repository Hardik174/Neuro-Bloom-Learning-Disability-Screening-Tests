...
export function SpaceBackground() {
  ...
  useEffect(() => {
    ...
    const isInCenter = (x: number, y: number) => {
      return Math.abs(x - centerX) < avoidCenterRadiusX && 
             Math.abs(y - centerY) < avoidCenterRadiusY
    }
    ...
    for (let i = 0; i < 6; i++) { // fewer planets
      ...
      let surfaceFeatures: { offsetX: number, offsetY: number, radius: number, color: string }[] = []
        ...
        if (planet.radius > 25) {
          ...
          if (planet.surfaceType === "bands") {
            ...
            for (let s = 0; s < planet.surfaceFeatures.length; s++) {
              ...
              ctx.beginPath()
              ctx.arc(
                f.offsetX * planet.radius,
                f.offsetY * planet.radius,
                planet.radius * f.radius,
                0, Math.PI * 2
              )
              ctx.fill()
            }
      ...
      spaceships.forEach((ship) => {
        ...
        if (ship.type === 0) {
          ...
          const cockpitGradient = ctx.createRadialGradient(
            ship.size * 0.1, 0, 0,
            ship.size * 0.1, 0, ship.size * 0.25
          )
          ...
          const thrusterGradient = ctx.createLinearGradient(
            -ship.size * 0.4, 0,
            -ship.size * (0.9 + ship.thrusterFlicker * 0.6), 0
          )
          ...
          if (Math.random() > 0.97) {
            const tractorGradient = ctx.createRadialGradient(
              0, 0, 0,
              0, ship.size * 2, ship.size * 2
            )
          ...
          const flameGradient = ctx.createLinearGradient(
            0, ship.size * 0.3,
            0, ship.size * (0.7 + ship.thrusterFlicker * 0.4)
          )
    ...
    function shadeColor(color: string, percent: number) {
      ...
      let hex = color.replace("#", "").substring(0, 6)
